      ...
      <div class="wm-login_body">
        <div class="wm-login_ct">
          ...
          <div class="wm-login_form" v-else-if="login.is_safety">
            ...
            <input ref="loginVcode" type="text" class="input" v-model="login.vcode" @keyup.enter="clickLogin()" maxlength="4" placeholder="请输入验证码">
          </div>
          <div class="wm-login_text">
            <span v-if="login.uname" @click="clearUser()">切换用户</span>
...
.wm-login_mask{position: absolute; z-index: 2; width: 100%; height: 100%; background: -webkit-radial-gradient(50% 10% ,farthest-corner,rgba(0, 100, 200, 0),rgba(0, 100, 200, 0.8),#0064C8,#172E8C);}
.wm-login_body{position: absolute; z-index: 3; width: 100%; height: 100%; color: @Minor6;}
.wm-login_ct{position: absolute; min-width: 320px; left: 50%; top: 50%; transform: translate(-50%, -60%);}
...
.wm-login_text span{cursor: pointer; padding: 8px 16px; color: @Minor6;}
</style>
