<template>
  ...
  <div class="app_ct">
    <!-- List -->
   <wm-table ref="tableList" overflow="auto" :columns="list.columns" :options="list.data" @orderBy="orderBy" @partially="selectState" height="calc(100% - 58px)">
      <template #id="d">
...
const sea = ref({
  ...
  columns:[
    {label: langs.web_html_title, value: '', name: 'title'},
    {label: langs.web_html_name, value: '', name: 'name'},
  ],
});
// 列表
...
const list = ref({columns: [
  ...
  {title: langs.web_html_type, index: 'type', slot: 'type', order: '', width: '80px', minWidth: '80px', textAlign: 'center'},
  {title: langs.web_html_title, index: 'title', order: '', width: '200px', minWidth: '160px'},
  {title: langs.web_html_name, index: 'name', order: '', width: '120px', minWidth: '120px'},
  ...
  {title: langs.ctime, index: 'ctime', order: '', width: '160px', minWidth: '160px'},
  {title: langs.utime, index: 'utime', order: '', width: '160px', minWidth: '160px'},
  {title: langs.remark, index: 'remark'},
], data: [], num: 0, total: 0, order: ''});
const page = ref({total: 0, num: 1, limit: 100});
// 添加&编辑、删除、导出
const save = ref({show: false, type: '', title: '添加/编辑', data: <any>{}});
...
const loadData = (): void => {
  ...
  Request.Post('web_html/list?lang=' + state.lang, {
    token: state.token,
    data: getWhere(),
    page: page.value.num,
  ...
  // 统计
  page.value.total = 0;
  Request.Post('web_html/total?lang='+state.lang, {
    token: state.token,
    data: getWhere(),
  }, (res:any)=>{
    const {code, msg, time, data}: any = res.data;
