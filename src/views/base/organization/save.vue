  ...
  <wmDialog v-model:show="infoShow" :title="title" width="720px" bottom="40px" @close="close()">
    ...
    <wmMain>
      <wmTableForm>
        ...
        <tr>
          <td class="label"><span class="red_dot">FID</span></td>
          <td>
            <wmInput v-model:value="form.fid" maxlength="4"></wmInput>
          </td>
...
// 数据
const form = ref({id: 0, status: true, fid:'0', sort:'0', name: '', remark: ''});
...
watch(()=>props.show, (val: boolean)=>{
  ...
  if(val){
    ...
    form.value.fid = props.data.fid || '0';
...
const verify = (form: any): any => {
  if(isNaN(form.fid)) return Ui.Toast('请输入FID');
...
const submit = (): void => {
  ...
  Request.Post('erp_base_organization/save?lang='+state.lang, {
    token: state.token,
    data: data,
  }, (res:any)=>{
    load.clear();
