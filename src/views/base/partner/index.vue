  ...
  <div class="app_action flex">
    ...
    <div class="app_action_search flex">
      ...
      <wmSearch v-model:show="sea.show" v-model:keys="sea.key" :columns="sea.columns" @keyup.enter="page.num=1;loadData()" @search="page.num=1;loadData()" @reset="resetData()">
        <template #type="d">
          <wmSelect v-model:value="sea.type" :options="selectAll.type_name" :placeholder="d.label" clearable multiple></wmSelect>
        </template>
        <template #class="d">
  ...
  <div class="app_ct">
   ...
   <wmTable ref="tableList" overflow="auto" :columns="list.columns" :options="list.data" @orderBy="orderBy" @partially="selectState">
      ...
      <template #wms_co_id="d">
        <div class="tCenter"><b>{{ d.wms_co_id }}</b></div>
      </template>
      <template #sort="d">
...
const list = ref({columns: [
  ...
  {title: '类型', slot: 'type', index: 'type', order: '', textAlign: 'center', width: '60px', minWidth: '60px'},
...
// 全部分类
const selectAll = ref({type_name: <any>[], class_name: <any>[], status_name: <any>[]});
...
const loadData = (): void => {
  ...
  Request.Post('erp_base_partner/list?lang=' + state.lang, {
    token: state.token,
    data: getWhere(),
    page: page.value.num,
  ...
  // 统计
  page.value.total = 0;
  Request.Post('erp_base_partner/total?lang=' + state.lang, {
    token: state.token,
    data: getWhere(),
  }, (res: any) => {
    const { code, msg, time, data }: any = res.data;
