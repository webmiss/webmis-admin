  ...
  <wmDialog v-model:show="infoShow" :title="title" width="720px" bottom="40px" @close="close()">
    ...
    <wmMain>
      <wmTableForm>
        ...
        <tr>
          <td class="label"><span class="red_dot">分仓ID</span></td>
          <td>
            <wmInput v-model:value="form.wms_co_id" maxlength="8"></wmInput>
          </td>
...
// 数据
const form = ref({id: 0, status: true, type:<any>'', class:<any>'', sort:'0', wms_co_id:'', name: '', remark: ''});
// 全部分类
const selectAll = ref({type_name: <any>[], class_name: <any>[]});
...
const getSelect = (): void => {
  form.value.type = [];
  form.value.class = [];
  Request.Post('erp_base_partner/get_select?lang='+state.lang, {
    token: state.token,
  }, (res:any)=>{
...
const verify = (form: any): any => {
  if(form.type.length<1) return Ui.Toast('请选择城市');
...
const submit = (): void => {
  ...
  Request.Post('erp_base_partner/save?lang='+state.lang, {
    token: state.token,
    data: data,
  }, (res:any)=>{
    load.clear();
