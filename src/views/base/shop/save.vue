    ...
    <wmMain>
      <wmTableForm>
        <tr>
          ...
          <td class="label"><span class="red_dot">所属</span></td>
          <td>
            <wmInput v-model:value="form.fid" maxlength="16" placeholder=":FID:FID:"></wmInput>
          </td>
        </tr>
        <tr>
          <td class="label"><span class="red_dot">城市</span></td>
          <td>
            <wmSelect v-model:value="form.city" :options="selectAll.city_name"></wmSelect>
          </td>
        ...
        <tr>
          <td class="label"><span class="red_dot">店铺ID</span></td>
          <td>
            <wmInput v-model:value="form.shop_id" maxlength="8"></wmInput>
          </td>
        ...
        <tr>
          ...
          <td colspan="3">
            <wmInput v-model:value="form.name" maxlength="32"></wmInput>
          </td>
        </tr>
        <tr>
...
// 数据
const form = ref({id: 0, status: true, city:<any>'', class:<any>'', fid:'', sort:'0', shop_id:'', name: '', remark: ''});
// 全部分类
const selectAll = ref({city_name: [], class_name: []});
...
watch(()=>props.show, (val: boolean)=>{
  ...
  if(val){
    ...
    form.value.fid = props.data.fid || '';
    form.value.sort = props.data.sort || '0';
    form.value.shop_id = props.data.shop_id || '';
...
const getSelect = (): void => {
  ...
  Request.Post('erp_base_shop/get_select?lang='+state.lang, {
    ...
    if(code==0) {
      ...
      // 默认值
      nextTick(()=>{
        form.value.city = typeof props.data.city!='undefined'?[props.data.city]:'';
...
const verify = (form: any): any => {
  if(form.fid.length<1) return Ui.Toast('请输入所属事业部');
  if(form.city.length<1) return Ui.Toast('请选择城市');
  if(form.class.length<1) return Ui.Toast('请选择分类');
  if(isNaN(form.sort)) return Ui.Toast('请输入排序数字');
  if(form.name.length<2 || form.name.length>32) return Ui.Toast('名称2～32字符!');
  return form;
}
...
const submit = (): void => {
  ...
  Request.Post('erp_base_shop/save?lang='+state.lang, {
    token: state.token,
    data: data,
  }, (res:any)=>{
    load.clear();
