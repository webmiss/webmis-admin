    ...
    <wmTable ref="tableList" overflow="auto" :columns="list.columns" :options="list.data" @orderBy="orderBy" @partially="selectState">
      ...
      <template #status="d">
        <div v-if="d.status=='0'" class="flex_center status0">
          <wmLabel :options="dayDiff(d.utime)">{{ d.state_name }}</wmLabel>
        </div>
...
// 操作
const push = ref({ show: false, title: '确认收货', info: '', num: 0, data: <any>[] });
...
const loadData = (): void => {
  ...
  Request.Post('erp_purchase_allocate_in/list?lang=' + state.lang, {
    token: state.token,
    data: getWhere(),
    page: page.value.num,
  ...
  Request.Post('erp_purchase_allocate_in/total?lang=' + state.lang, {
    token: state.token,
    data: getWhere(),
  }, (res: any) => {
    const {code, msg, data, time}: any = res.data;
...
const diffData = (): void => {
  ...
  diff.value.title = '对比调拨出( '+diff.value.data.join(',')+' )';
}
...
const getSelect = (): void => {
  Request.Post('erp_purchase_allocate_in/get_select?lang=' + state.lang, {
    token: state.token,
  }, (res: any) => {
    const {code, msg, data}: any = res.data;
    if(code===0){
