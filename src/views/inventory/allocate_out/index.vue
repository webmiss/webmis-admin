   ...
   <wmTable ref="tableList" overflow="auto" :columns="list.columns" :options="list.data" @orderBy="orderBy" @partially="selectState">
      ...
      <template #action="d">
        <div class="tCenter">
          ...
          <wmButton type="danger" effect="text" @click="revokeData(d.id, d.go_co_name, d.link_co_name, d.num)" v-else-if="isAction('save')&&d.state=='1'">撤回</wmButton>
...
const goodsData = (row: any): void => {
  ...
  goods.value.data.state = row.state;
  goods.value.data.level = row.level;
}
/* 商品-回调 */
