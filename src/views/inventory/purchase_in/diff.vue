<template>
    ...
    <template #top>
      ...
      <div class="goods_info flex">
        <ul class="goods_info_tools flex">
          <li>
            <wmInput v-model:value="goods.key" @keyup.enter="goodsSearch()" @iconClick="goodsSearch()" placeholder="采退单号,多个空格分离" maxlength="32" icon="ui ui_search" iconAlign="right" padding="0 40px 0 10px"></wmInput>
          </li>
          <li>
            <wmButton type="primary" effect="text" padding="0 4px" @click="form.show=true;formData(form.key)">选择采退单</wmButton>
          </li>
        </ul>
        <div class="goods_info_total scrollbar" style="max-width: calc(100% - 600px);">
          入库数: <b>{{ num }}</b> &nbsp;&nbsp;
          采退数: <b>{{ goods.out_num }}</b> &nbsp;&nbsp;
  ...
  <!-- 选择采退单 -->
  <wmDialog v-model:show="form.show" title="选择采退单" width="calc(100% - 40px)" maxWidth="1280px" overflow="auto" top="56px" bottom="40px" @close="form.show=false">
    <template #top>
      <!-- 工具栏 -->
      <div class="goods_info flex">
        <ul class="goods_info_tools flex">
...
const formSubmit = (): void => {
  ...
  if(list.length==0) return Ui.Toast('请选择采退单');
  let ids: Array<any> = [];
