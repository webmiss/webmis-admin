<template>
  <wmDialog v-model:show="infoShow" :title="title" width="calc(100% - 40px)" top="56px" bottom="0px" overflow="auto" :bgClose="false" @close="close()">
    <template #top>
      <!-- 工具栏 -->
...
const goodsTotal = (refresh: boolean): void => {
  ...
  for(let v of list) {
    ...
    goods.value.num += num;
    // 标记编码
    v.is_date = sku_tmp.substring(0, 7) != v.sku_id.substring(0, 7);
  }
  // 刷新
