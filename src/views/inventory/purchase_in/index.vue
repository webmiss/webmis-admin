  ...
  <div class="app_ct">
    ...
    <wmTable ref="tableList" overflow="auto" :columns="list.columns" :options="list.data" @orderBy="orderBy" @partially="selectState">
      ...
      <template #sale_price="d">
        <div class="tRight">
          {{ d.sale_price>0?priceEncode(d.sale_price):"-" }}
        </div>
      </template>
      <template #market_price="d">
        <div class="tRight">
          {{ d.market_price>0?priceEncode(d.market_price):"-" }}
        </div>
      </template>
      <template #num="d">
      ...
      <template #action="d">
        <div class="tCenter">
          ...
          <wmButton type="danger" effect="text" @click="revokeData(d.id, d.brand, d.num)" v-else-if="isAction('save') && d.state=='1'">撤回</wmButton>
...
const loadData = (): void => {
  ...
  Request.Post('erp_purchase_in/total?lang=' + state.lang, {
    token: state.token,
    data: getWhere(),
  }, (res: any) => {
    const {code, msg, data, time}: any = res.data;
...
const diffData = (): void => {
  ...
  diff.value.title = '对比采退单( '+diff.value.data.join(',')+' )';
}
