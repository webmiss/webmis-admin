  ...
  <div class="app_action flex">
    <div class="app_action_tools scrollbar">
      <wmAction :columns="[
        ...
        <template #push>
          <wmButton effect="plain" type="primary" icon="ui ui_safety" padding="0 16px 0 8px" :disabled="push.data.length==0" @click="pushData()">推送({{ push.data.length }})</wmButton>
        </template>
        <template #line2>
   ...
   <wmTable ref="tableList" overflow="auto" :columns="list.columns" :options="list.data" @orderBy="orderBy" @partially="selectState">
      ...
      <template #status="d">
        <div v-if="d.status=='0'" class="flex_center status0">
          <wmLabel :options="dayDiff(d.ctime)">{{ d.status_name }}</wmLabel>
        </div>
      ...
      <template #action="d">
        <div class="tCenter">
          <wmButton @click="saveData('edit', d)" v-if="isAction('save')&&d.state=='0'">{{ langs.edit }}</wmButton>
          <wmButton type="danger" effect="text" @click="revokeData(d.id, d.num)" v-else-if="isAction('save')&&d.state=='1'">撤回</wmButton>
...
const list = ref({ columns: [
  ...
  { title: '退货仓库', index: 'wms_co_name', width: '200px' },
...
const push = ref({ show: false, title: '推送', info: '', num: 0, data: <any>[] });
...
const loadData = (): void => {
  ...
  Request.Post('erp_purchase_out/list?lang=' + state.lang, {
    token: state.token,
    data: getWhere(),
    page: page.value.num,
  ...
  Request.Post('erp_purchase_out/total?lang=' + state.lang, {
    token: state.token,
    data: getWhere(),
  }, (res: any) => {
    const {code, msg, data, time}: any = res.data;
...
/* 撤回 */
const revokeData = (id: number, num: number): void => {
  revoke.value.show = true;
  revoke.value.data = [id];
  revoke.value.info = '单号: <b>' + id + '</b>&nbsp;&nbsp;&nbsp;&nbsp;数量: <b>' + num + '</b>';
}
/* 撤回-回调 */
...
const diffData = (): void => {
  ...
  diff.value.title = '对比入库单( '+diff.value.data.join(',')+' )';
}
...
const goodsData = (row: any): void => {
  ...
  goods.value.data.brand = row.brand;
}
/* 商品-回调 */
...
const getSelect = (): void => {
  Request.Post('erp_purchase_out/get_select?lang=' + state.lang, {
    token: state.token,
  }, (res: any) => {
    const {code, msg, data}: any = res.data;
    if(code===0){
