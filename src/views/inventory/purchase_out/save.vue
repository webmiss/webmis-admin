<template>
  <wmDialog v-model:show="infoShow" :title="title" width="620px" bottom="40px" @close="close()">
    <wmMain>
      <wmTableForm>
        <tr>
          <td class="label"><span class="red_dot">退货仓库</span></td>
          <td>
            <wmSelect v-model:value="form.wms_co_id" :options="selectAll.partner_name" :disabled="form.tp=='edit'" />
          </td>
        </tr>
        <tr>
          <td class="label"><span class="red_dot">退货类型</span></td>
          <td>
...
const verify = (form: any): any => {
  if(!form.wms_co_id || form.wms_co_id.length==0) return Ui.Toast('请选择退货仓库');
  if(!form.type || form.type.length==0) return Ui.Toast('请选择退货类型');
...
const submit = (): void => {
  ...
  Request.Post('erp_purchase_out/save?lang='+state.lang, {
    token: state.token,
    data: data,
  }, (res:any)=>{
    load.clear();
