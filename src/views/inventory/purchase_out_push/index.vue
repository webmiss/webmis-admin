  ...
  <div class="app_action flex">
    <div class="app_action_tools scrollbar">
      <wmAction :columns="[
        ...
        <template #push>
          <wmButton effect="dark" type="primary" icon="ui ui_safety" padding="0 16px 0 8px" :disabled="push.data.length==0" @click="pushData()">确认退货({{ push.data.length }})</wmButton>
        </template>
        <template #line>
   ...
   <wmTable ref="tableList" overflow="auto" :columns="list.columns" :options="list.data" @orderBy="orderBy" @partially="selectState">
      ...
      <template #status="d">
        <div v-if="d.status=='0'" class="flex_center status0">
          <wmLabel :options="dayDiff(d.utime)">{{ d.status_name }}</wmLabel>
        </div>
      ...
      <template #action="d">
        <div class="tCenter">
          <wmButton type="danger" effect="text" @click="revokeData(d.id, d.num)" v-if="isAction('revoke')&&d.state=='1'">撤回</wmButton>
...
// 操作
const push = ref({ show: false, title: '确认退货', info: '', num: 0, data: <any>[] });
...
const loadData = (): void => {
  ...
  Request.Post('erp_purchase_out_push/list?lang=' + state.lang, {
    token: state.token,
    data: getWhere(),
    page: page.value.num,
  ...
  Request.Post('erp_purchase_out_push/total?lang=' + state.lang, {
    token: state.token,
    data: getWhere(),
  }, (res: any) => {
    const {code, msg, data, time}: any = res.data;
...
const printData = (row: any): void => {
  ...
  // 明细
  const load: any = Ui.Loading();
  Request.Post('erp_purchase_out_push/goods_list?lang=' + state.lang, {
    token: state.token,
    id: row.id,
    ctime: row.ctime,
    utime: row.utime,
...
const getSelect = (): void => {
  Request.Post('erp_purchase_out_push/get_select?lang=' + state.lang, {
    token: state.token,
  }, (res: any) => {
    const {code, msg, data}: any = res.data;
    if(code===0){
