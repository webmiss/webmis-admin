<template>
  <wmDialog v-model:show="infoShow" :title="title" width="calc(100% - 40px)" maxWidth="1600px" top="56px" bottom="0px" overflow="auto" :bgClose="false" @close="close()">
    <template #top>
      <!-- 工具栏 -->
      ...
      <wmTable ref="tableList" :columns="goods.columns" :options="goods.list" @orderBy="orderBy" @partially="selectState" :maxLen="100">
        ...
        <template #action="d">
          <div class="flex_center">
            ...
            <template v-else>
              <span v-if="d.status==0" class="c_danger">待确认</span>
  ...
  <!-- Action -->
  <actionImport v-model:show="imp.show" :title="imp.title" :data="{type: props.data.type, wms_co_id: props.data.wms_co_id}" @submit="importSubmit($event)"></actionImport>
  <!-- 图片预览 -->
...
const goodsAddData = (data: Array<any>): void => {
  let tmp: any = data;
  Request.Post('erp_order_inout/goods_add?lang=' + state.lang, {
    token: state.token,
    id: props.data.id,
...
const goodsNum = (e: any, id: string, sku_id: string): void => {
  ...
  // 请求
  Request.Post('erp_order_inout/goods_num?lang=' + state.lang, {
    token: state.token,
    id: id,
    num: num,
    sku_id: sku_id,
...
const goodsRemoveSub = (): void => {
  ...
  const load: any = Ui.Loading();
  Request.Post('erp_order_inout/goods_remove?lang=' + state.lang, {
    token: state.token,
    id: props.data.id,
    ctime: props.data.ctime,
    wms_co_id: props.data.wms_co_id,
...
const close = (): void => {
  ...
  if(goods.value.refresh) {
    const load: any = Ui.Loading();
    Request.Post('erp_order_inout/goods_price?lang=' + state.lang, {
      token: state.token,
      id: props.data.id,
      ctime: props.data.ctime,
    }, (res: any) => {
